...
def seeded_db(db_path):
    ...
    conn.execute("INSERT OR REPLACE INTO dim_session (session_id, meeting_key, session_type, session_name, total_laps) VALUES ('2024_21_R', '2024_21', 'R', 'Brazil GP Race', 69)")
    conn.close()
    return db_path
...
def test_check_prerequisites_raises_without_laps(db_path):
    conn = duckdb.connect(str(db_path))
    conn.execute("INSERT OR REPLACE INTO dim_session (session_id, meeting_key, session_type, session_name, total_laps) VALUES ('2024_21_R', '2024_21', 'R', 'test', 69)")
    conn.close()
