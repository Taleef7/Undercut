...
def test_normalize_pit_stops_loads_fixture(db_path):
    n = normalize_pit_stops(db_path, data_dir=Path("tests/fixtures"), meeting_key=47, session_key=9540)
    assert n == 1
    conn = duckdb.connect(str(db_path))
    df = conn.execute("SELECT * FROM fact_pit_stop").fetchdf()
    conn.close()
    assert len(df) == 1


def test_normalize_pit_stops_returns_zero_for_missing_data(db_path):
    n = normalize_pit_stops(db_path, data_dir=Path("/nonexistent"), meeting_key=47, session_key=9540)
    assert n == 0


def test_normalize_pit_stops_has_record_hashes(db_path):
    n = normalize_pit_stops(db_path, data_dir=Path("tests/fixtures"), meeting_key=47, session_key=9540)
    conn = duckdb.connect(str(db_path))
    df = conn.execute("SELECT record_hash FROM fact_pit_stop").fetchdf()
    conn.close()
